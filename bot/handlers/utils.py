    ...
    while (True):
        ...
        if (r.status_code == 200):
            ...
            try:
                ...
                product = products[i]
                url = product.find(
                    'a', class_='styles_wrapper__yaLfq').get('href')
                url = url.split('?')[0]
...
def register_handlers_client(dp: Dispatcher):
    dp.register_message_handler(client.start_command, commands='start')
    dp.register_message_handler(client.search_command, commands='search')
    dp.register_message_handler(client.stop_command, commands='stop')
